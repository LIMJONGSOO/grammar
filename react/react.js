...
컴포넌트
 ...
 - props
  ...
  함수형 컴포넌트 - const { name, children } = props; 혹은 파라미터 부분에서 const MyComponent = ({name, children}) => {}
                    propType - propsType을 선언해서 타입을 지정할 수 있음, 다른 타입을 넘기는 경우 오류발생
                    name: PropTyps.string.isRequired  필수값 지정
  클래스형 컴포넌트 - this.props로 사용가능
...
이벤트 핸들링
  클래스형 컴포넌트 - this.setState({[e.target.name]:e.target.value});
  함수형 컴포넌트 - 1. setName(e.target.value); 속성별로 선언필요
                   2. const [form, setForm] = useState({name: '',...});
